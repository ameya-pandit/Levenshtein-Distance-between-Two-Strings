...
//THE FOLLOWING IS MY CODE FROM HW2 - HERE, I AM IMPLEMENTING A DOUBLY LINKED LIST, WHICH IS A SUITABLE DATA STRUCTURE THAT ALLOWS ALL 4 OPERATIONS TO HAPPEN IN O(1) TIME.
...
/*
 ...
 void DList::remove(Node *node)  {
     if (node != NULL)    {
         ...
         if (head == node)   {  //front of list
             head = node->next;
         }
         if (tail == node) {   //back of list
             tail = node->prev;
         }
         if (node->next != NULL) {
 ...
 Node *DList::previous(Node *node)  {
     return node->prev;
 }

 Node *DList::next(Node *node) {
     return node->next;
 }

 Node *DList::search_value(int value) {
     Node *temp = head;
